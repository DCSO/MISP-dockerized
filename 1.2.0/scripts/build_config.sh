...
func_check_if_vars_exists() {
  ...
  # END DEPRECATED
  # LOG_SETTINGS
  [ -z "${SYSLOG_QUESTION_USE_SYSLOG+x}" ]      && [ -z "${SYSLOG_REMOTE_HOST}" ] && SYSLOG_REMOTE_HOST="127.0.0.1" && QUERY_LOG_SETTINGS="yes"
  [ "${SYSLOG_QUESTION_USE_SYSLOG+x}" = "yes" ] && [ -z "${SYSLOG_REMOTE_HOST}" ] && SYSLOG_REMOTE_HOST="127.0.0.1" && QUERY_LOG_SETTINGS="yes"
...
services:
  ...
  misp-server:
    ...
    environment:
      ...
      MAIL_DEBUG_PEERS: "${MAIL_DEBUG_PEERS}"
