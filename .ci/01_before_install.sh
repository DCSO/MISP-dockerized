...
echo "$STARTMSG Install requirements..."
    [ ! -z "$(command -v apk)" ] && apk add --no-cache bash sudo git curl coreutils grep py-pip python-dev libffi-dev openssl-dev gcc libc-dev make
    [ ! -z "$(command -v apt-get)" ] && apt-get update; 
    [ ! -z "$(command -v apt-get)" ] && apt-get install make bash sudo git curl coreutils grep python3 gcc
    # Upgrade Docke
